function getDataByWindowUrlKey(){
  ...
  let url = baseUrl+'/api/sneaker/'+windowUrlKey;
  ...
  .then(function (response) {
    ...
    $('#product-author').html(response.data.shoe_designer);
    ...
    // START -- note, unless you have these data in a database structure, here we are hardcoding them for display purposes
      ...
      // tag
      let collectionOfTag = ['Shoes', 'Sneakers', 'Casual Shoes', 'Sport Shoes', 'High Quality','New Arrivals', 'Stylish'];
  ...
  .catch(function (error) {
    ...
    if(error.code == "ERR_BAD_REQUEST"){
      Swal.fire({
        ...
        icon: "warning",
        title: "Yaah...",
        html: "Produk yang lagi dicari tidak ditemukan",
